...
func TestKeystore_ListEnvironments(t *testing.T) {
	...
	// Verify all environments are present and sorted alphabetically
	expectedEnvs := []string{"development", "production", "staging"}
	for i, env := range environments {
		if env != expectedEnvs[i] {
			t.Errorf("Environment[%d] = %v, want %v", i, env, expectedEnvs[i])
		}
	}
